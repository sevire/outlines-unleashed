...
class ContainerExtractor(object):
  ...
  def __init__(self, config):
    """The config parameter is a map containing three lists: allowed, copied and
    extracted.
...
class Globable(object):
  """A bit of text which can be globbed (lumped together in bits). Methods
  current(), skipcurrent(), checkfor() and isout() have to be implemented by
  subclasses.
  """
...
class Position(Globable):
  """A position in a text to parse. Including those in Globable, functions to
  implement by subclasses are: skip(), identifier(), extract(), isout() and
  current().
  """
...
class Formula(Container):
  ...
  def googlecharts(self):
    """Make the contents using Google Charts
...
class NumberGenerator(object):
  """A number generator for unique sequences and hierarchical structures. Used
  in:
  """
...
class FormulaCommand(FormulaBit):
  ...
  def emptycommand(self, pos):
    """Check for an empty command: look for command disguised as ending. Special
    case against '{ { } }' situation.
...
class HybridFunction(ParameterFunction):
  """A parameter function where the output is also defined using a template. The
  template can use a number of functions; each function has an associated tag.
  Example: [f0{$1},span class="fbox"] defines a function f0 which corresponds to
  a span of class fbox, yielding <span class="fbox">$1</span>. Literal
  parameters can be used in tags definitions:

      [f0{$1},span style="color: $p;"]

  yields <span style="color: $p;">$1</span>, where $p is a literal parameter.
  Sizes can be specified in hybridsizes, e.g. adding parameter sizes. By default
  the resulting size is the max of all arguments. Sizes are used to generate the
  right parameters. A function followed by a single / is output as a
  self-closing XHTML tag:
  ...
  def readtag(self, pos):
    """Get the tag corresponding to the given index. Does parameter
    substitution.
