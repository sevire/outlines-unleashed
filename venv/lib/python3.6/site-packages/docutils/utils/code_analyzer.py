...
class Lexer(object):
    ...
    Arguments
        ...
        code -- string of source code to parse, language -- formal language the
        code is written in, tokennames -- either 'long', 'short', or '' (see
        below).
    ...
    tuples. The value of `tokennames` configures the naming of the tokentype:
        ...
        'long': downcased full token type name, 'short': short name defined by
...
class NumberLines(object):
    ...
    Arguments
        ...
        tokens -- iterable of ``(classes, value)`` tuples startline -- first
        line number endline -- last line number
