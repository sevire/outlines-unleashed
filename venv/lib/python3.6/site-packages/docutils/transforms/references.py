...
class PropagateTargets(Transform):
    ...
    PropagateTargets propagates the ids and names of the internal targets
    preceding the paragraph to the paragraph itself::
...
class IndirectHyperlinks(Transform):
    ...
    """a) Indirect external references::
       ...
       The "refuri" attribute is migrated back to all indirect targets from the
       final direct target (i.e. a target not referring to another indirect
       target)::
       ...
       Once the attribute is migrated, the preexisting "refname" attribute is
       dropped.
    ...
    b) Indirect internal references::
       ...
       target's "id"). References which indirectly refer to an internal target
       become direct internal references::
...
class InternalTargets(Transform):
    ...
    def resolve_reference_ids(self, target):
        ...
        The "refname" attribute is replaced by "refid" linking to the
        target's "id"::
...
class Footnotes(Transform):
    ...
    Auto-numbered footnote_references have no reference text (they're empty
    elements). When resolving the numbering, a ``label`` element is added to the
    beginning of the ``footnote``, and reference text to the
    ``footnote_reference``.
    ...
    their references via "refids" attributes. References are assigned "id" and
    "refid" attributes.
...
class Substitutions(Transform):
    ...
    The ``substitution_reference`` will simply be replaced by the contents of
    the corresponding ``substitution_definition``.
...
class TargetNotes(Transform):
    ...
    """Creates a footnote for each external target in the text, and
