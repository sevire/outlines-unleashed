...
class CallBack(Transform):
    ...
    node. Other data can be stored in the ``details`` attribute or in the object
    hosting the callback method.
    """
...
class Transitions(Transform):
    ...
    transitions after a title, at the beginning or end of the document, and
