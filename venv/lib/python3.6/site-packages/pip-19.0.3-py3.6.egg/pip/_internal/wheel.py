...
def sorted_outrows(outrows):
    ...
    Each row is a 3-tuple (path, hash, size) and corresponds to a record of a
    RECORD file (see PEP 376 and PEP 427 for details). For the rows passed to
    this function, the size can be an integer as an int or string, or the empty
    string.
...
def get_csv_rows_for_installed(
    ...
    Args:
        old_csv_rows:
        installed: A map from archive RECORD path to installation RECORD path.
        changed:
        generated:
        lib_dir:
    """
...
def check_compatibility(version, name):
    ...
    Pip should refuse to install a Wheel-Version that's a major series ahead
    of what it's compatible with (e.g 2.0 > 1.1); and warn when installing a
    version only minor version ahead (e.g 1.2 > 1.1).

    version: a 2-tuple representing a Wheel-Version (Major, Minor) name: name
    of wheel or package to raise exception about
    ...
    Raises:
        UnsupportedWheel: when an incompatible Wheel-Version is given
    """
    if not version:
...
class Wheel(object):
    ...
    def __init__(self, filename):
        ...
        Raises:
            InvalidWheelFilename: when the filename is invalid for a wheel
        """
...
def _contains_egg_info(
    ...
    Args:
        s: The string to parse. E.g. foo-2.1
        _egg_info_re:
    """
...
def should_use_ephemeral_cache(
    ...
    # type: (...) -> Optional[bool]
    """Return whether to build an InstallRequirement object using the ephemeral
    cache.
    ...
    Args:
        ...
        format_control:
        autobuilding:
        cache_available: whether a cache directory is available for the
            autobuilding=True case.

    Returns:
        True or False to build the requirement with ephem_cache=True or False,
        respectively; or None not to build the requirement.
    """
    if req.constraint:
        return None
    if req.is_wheel:
...
class WheelBuilder(object):
    ...
    def _build_one(self, req, output_dir, python_tag=None):
        ...
        Returns:
            The filename of the built wheel, or None if the build failed.
        """
    ...
    def build(
        ...
        Returns:
            True if all the wheels built correctly.
        """
        buildset = []
