...
def display_path(path):
    ...
    """Gives the display value for a given path, making it relative to cwd if
    possible.
...
def has_leading_dir(paths):
    ...
    """Returns true if all the paths have the same leading path name (i.e.,
    everything is in one subdirectory in an archive)
...
def dist_is_local(dist):
    ...
    """Return True if given Distribution object is installed locally (i.e.
    within current virtualenv).
...
def get_installed_distributions(local_only=True,
    ...
    If ``local_only`` is True (default), only return installations local to
    the current virtualenv, if in a virtualenv.

    ``skip`` argument is an iterable of lower-case project names to ignore;
    ...
    If ``user_only`` is True , only report installations in the user site
    directory.
...
def egg_link_path(dist):
    ...
    There's 3 scenarios: 1) not in a virtualenv

        try to find in site.USER_SITE, then site_packages
    1) in a no-global virtualenv try to find in site_packages
    2) in a yes-global virtualenv try to find in site_packages, then
       site.USER_SITE (don't look in global location)
...
def dist_location(dist):
    ...
    """Get the site-packages location of this distribution. Generally this is
    dist.location, except in the case of develop-installed packages, where
    dist.location is the source code location, and we want to know where the
    egg-link file is.
...
def call_subprocess(
    ...
    Args:
        ...
        command_desc:
        extra_environ:
        unset_environ: an iterable of environment variable names to unset prior
            to calling subprocess.Popen().
        spinner:
    """
...
class FakeFile(object):
    """Wrap a list of lines in an object with readline() to make ConfigParser
    happy.
    """
...
def captured_output(stream_name):
    """Return a context manager used by captured_stdout/stdin/stderr that
...
def captured_stdout():
    """Capture the output of sys.stdout:
        ...
        self.assertEqual(stdout.getvalue(), 'hellon')
