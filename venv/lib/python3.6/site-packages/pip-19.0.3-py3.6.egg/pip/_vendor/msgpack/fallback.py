...
def unpackb(packed, **kwargs):
    ...
    Raises `ExtraData` when `packed` contains extra bytes. See
    :class:`Unpacker` for options.
...
class Unpacker(object):
    ...
    Args:
        file_like: File-like object having `.read(n)` method. If specified,
            unpacker reads serialized data from it and :meth:`feed()` is not
            usable.
        read_size (int): Used as `file_like.read(read_size)` . (default:
            `min(16*1024, max_buffer_size)` )
        use_list (bool): If true, unpack msgpack array to Python list.
            Otherwise, unpack to Python tuple. (default: True)
        raw (bool): If true, unpack msgpack raw to Python bytes (default).
            Otherwise, unpack to Python str (or unicode on Python 2) by decoding
            with UTF-8 encoding (recommended). Currently, the default is true,
            but it will be changed to false in near future. So you must specify
            it explicitly for keeping backward compatibility.

            *encoding* option which is deprecated overrides this option.
        object_hook (callable): When specified, it should be callable. Unpacker
            calls it with a dict argument after unpacking msgpack map. (See also
            simplejson)
        object_pairs_hook (callable): When specified, it should be callable.
            Unpacker calls it with a list of key-value pairs after unpacking
            msgpack map. (See also simplejson)
        encoding (str): Encoding used for decoding msgpack raw. If it is None
            (default), msgpack raw is deserialized to Python bytes.
        unicode_errors (str): (deprecated) Used for decoding msgpack raw with
            *encoding*. (default: `'strict'` )
        max_buffer_size (int): Limits size of data waiting unpacked. 0 means
            system's INT_MAX (default). Raises `BufferFull` exception when it is
            insufficient. You should set this parameter when unpacking data from
            untrusted source.
...
class Packer(object):
    ...
    usage:
        ...
        packer = Packer() astream.write(packer.pack(a))
        astream.write(packer.pack(b))

    Packer's constructor has some keyword arguments:

    Args:
        default (callable): Convert user type to builtin type that Packer
            supports. See also simplejson's document.
        use_single_float (bool): Use single precision float type for float.
            (default: False)
        autoreset (bool): Reset buffer after each pack and return its content as
            `bytes` . (default: True). If set this to false, use `bytes()` to
            get content and `.reset()` to clear buffer.
        use_bin_type (bool): Use bin type introduced in msgpack spec 2.0 for
            bytes. It also enables str8 type for unicode.
        strict_types (bool): If set to true, types will be checked to be exact.
            Derived classes from serializeable types will not be serialized and
            will be treated as unsupported type and forwarded to default.
            Additionally tuples will not be serialized as lists. This is useful
            when trying to implement accurate serialization for python types.
        encoding (str): (deprecated) Convert unicode to bytes with this
            encoding. (default: 'utf-8')
        unicode_errors (str): Error handler for encoding unicode. (default:
            'strict')
    """
