...
def linux_distribution(full_distribution_name=True):
    ...
    * ``codename``: The result of :func:`distro.codename`.
    ...
    data sources than the original function, and that may lead to different data
    if the OS distribution is not consistent across multiple data sources it
...
def id():
    ...
    * a normalization of the ID is performed, based upon
      ...
      in the OS distributions. A common reason for an incompatible change is the
...
def name(pretty=False):
    ...
    If *pretty* is true, the version and codename are appended. (e.g. "CentOS
    Linux 7.1.1503 (Core)")
...
def version(pretty=False, best=False):
    ...
    If *best* is false, the first non-empty version number in priority order
    ...
    * the version number parsed from the "Description" attribute returned by the
      lsb_release command, if it follows the format of the distro release files.
...
def like():
    ...
    **Lookup hierarchy:**
    ...
    This information item is only provided by the os-release file. For
    details, see the description of the "ID_LIKE" attribute in the `os-release
    man page <http://www.freedesktop.org/software/systemd/man/os-release.html>`_
    .
    """
    return _distro.like()
...
def codename():
    ...
    example, openSUSE returns "x86_64". This function does not handle such cases
    in any special way and just returns the string it finds, if any.
...
def info(pretty=False, best=False):
    ...
    which information items are available in the underlying data sources. The
    values for the various keys are as follows:
...
def os_release_attr(attribute):
    ...
    * (string): Value of the information item, if the item exists. The empty
      string, if the item does not exist.
...
def lsb_release_attr(attribute):
    ...
    output data source of the current OS distribution.
...
class cached_property(object):
    ...
    underlying function and sets the value in `__dict__` so future accesses will
...
class LinuxDistribution(object):
    ...
    returns data about the current OS distribution (i.e. the distro this package
    runs on).

    Normally, it is not necessary to create additional instances of this
    class. However, in situations where control is needed over the exact data
    sources that are used, instances of this class can be created with a
    specific distro release file, or a specific os-release file, or without
    invoking the lsb_release command.
    """
    ...
    def __init__(self,
        ...
        * ``os_release_file`` (string): The path name of the `os-release file`_
          that is to be used as a data source.

          An empty string (the default) will cause the default path name to be
          used (see `os-release file`_ for details).

          If the specified or defaulted os-release file does not exist, the data
          source for the os-release file will be empty.

        * ``distro_release_file`` (string): The path name of the
          `distro release file`_ that is to be used as a data source.

          An empty string (the default) will cause a default search algorithm to
          be used (see `distro release file`_ for details).
        ...
        * ``os_release_file`` (string): The path name of the `os-release file`_
          that is actually used as a data source. The empty string if no distro
          release file is used as a data source.
        ...
        * ``include_uname`` (bool): The result of the ``include_uname``
          parameter. This controls whether the uname information will be loaded.
    ...
    def linux_distribution(self, full_distribution_name=True):
        ...
        Python's :func:`platform.linux_distribution`, supporting a subset of its
        parameters.
    ...
    def _parse_distro_release_content(line):
        ...
        Parameters: * line: Line from the distro release file. Must be a
        unicode string
