...
class TestOutlineNodeWalk(TestCase):
    ...
    def test_node_order_and_text_contents(self, index, text_value):
        """
        Don't test every row but use data driven check on what is at each position to allow future expansion.
        :return:
        """
        node_list = list(self.outline.list_all_nodes())
        length = len(text_value)
        if length > 0 :
            text = node_list[index][0].text[:length]  # Truncate to length of test string
        else:
            text = node_list[index][0].text  # Don't want to truncate if testing for empty string
    ...
    def test_node_order_and_note_contents(self, index, note_value):
        ...
        if length > 0 :
            note = node_list[index][0].note[:length]  # Truncate to length of test string
        else:
            note = node_list[index][0].note  # Don't want to truncate if testing for empty string
        self.assertEqual(note_value, note)

    @unpack
    @data(*ancestry_test_values)
    def test_ancestry(self, index, ancestry):
        """
        ...
        node_list = list(self.outline.list_all_nodes())
        self.assertEqual(ancestry, node_list[index][1])
