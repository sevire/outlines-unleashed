...
class TestOutlineNode(TestCase):
    ...
    def test_confirm_identity(self):
        """
        Note this test will not pass until re-factoring is complete to ensure that same OutlineNode object is returned
        for same outline element.  Some thinking to do before we can do this.
        :return:
        """
        outline = Outline.from_opml(os.path.join(tcfg.test_resources_root, 'opml-test-valid-01.opml'))
